...
class BaseService3 {
    ...
    let domainApi = "https://gist.githubusercontent.com/CanThaiLinh/"
    private var headers: HTTPHeaders {
        var headers = HTTPHeaders()
        headers.add(name: AppKey.contentType, value: "text/plain")
        headers.add(name: AppKey.contentType, value: "application/json")
        headers.add(name: AppKey.contentType, value: "application/xml")
        headers.add(name: AppKey.contentType, value: "text/https")
        headers.add(name: AppKey.accept, value: "application/json")
        headers.add(name: AppKey.accept, value: "text/plain")
        headers.add(name: AppKey.accept, value: "application/xml")
        headers.add(name: AppKey.accept, value: "text/https")
        return headers
    }
    ...
    func request<T: Decodable>(_ path: String,
        ...
        let domain = "\(domainApi)"
        let url = path.starts(with: "http") ? path : "\(domain)\(path)"
        self.alamofireManager.request(url, method: method, parameters: parameters, headers: self.headers)
            .validate(statusCode: 200..<500)
            .validate(contentType: ["application/xml", "application/json", "text/plain"])
            .responseDecodable(of: APIResponse<T>.self) { response in
                switch response.result {
                case let .success(data):
                    print("1====Success\(data)")
                            ...
                            if let data = response.data {
                                do {
                                    if let jsonData = try JSONSerialization.jsonObject(with: data, options: .allowFragments) as? [String:AnyObject] {
                                        code = code == 0 ? (jsonData["code"] as? Int ?? code) : code
                ...
                case let .failure(error):
                    ...
                    if let data = response.data {
                        do {
                            if let jsonData = try JSONSerialization.jsonObject(with: data, options: .allowFragments) as? [String:AnyObject] {
                                code = jsonData["code"] as? Int ?? code
                                message = jsonData["message"] as? String ?? ""
                                print("32====")
                                print(data)
                            }
